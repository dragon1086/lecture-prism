...
API 키 없이 먼저 전체 흐름을 실행하고, 같은 프로젝트에 **실데이터·실제 AI·KIS 모의투자·KIS 실전투자 매수 주문 경로**까지 연결합니다. 완성된 비법을 복사하는 수업이 아니라, 내 생각을 작동하는 시스템으로 만드는 수업입니다.

![내 전략이 KIS 매수 주문까지 이어지는 과정](docs/assets/readme/strategy-to-kis.png)
...
5. 같은 과정을 다음 영역에 반복한 뒤, 실데이터·실제 AI·KIS 매수 주문 경로까지 연결합니다.

| 수업 | 무엇을 완성하나 |
|---|---|
| **파트 3** | 후보 선정 → AI 분석 → 주문 판단 → 매매 기록이 이어지는 전체 시스템을 실행하고, 실데이터·실제 AI·KIS를 연결합니다. |
...
![후보 선정부터 KIS 주문과 매매 기록까지 이어지는 시스템](docs/assets/readme/system-result.png)

- **후보 선정**: 어떤 종목을 볼지 규칙으로 좁힙니다.
- **AI 분석**: 기술·뉴스·시장 흐름을 함께 보고 투자의견을 만듭니다.
- **주문 판단**: 매수 여부와 수량을 정하고, 손절·목표가 조건을 적용합니다.
- **KIS 주문**: 처음에는 안전하게 검증하고, 설정 후 모의·실전 계좌의 매수 주문 경로를 사용합니다.
- **매매 기록**: 판단과 결과를 저장해 다음 전략 수정의 근거로 씁니다.
...
- **KIS API로 이어지는 같은 프로젝트**: 안내에 따라 선택 기능을 준비하고, `.env`와 `trading/trading/config/kis_devlp.yaml`에 필요한 설정을 채우면 KIS 모의투자와 KIS 실전투자의 매수 주문 경로를 사용할 수 있습니다.
...
여러분도 집에서 AI와 함께 매매 로직을 다듬고, 모니터링과 대시보드를 보기 좋게 가꾸고, 텔레그램·디스코드 알림을 더 쓸모 있게 만들 수 있습니다. 한꺼번에 크게 바꾸기보다 한 가지를 고치고 확인하는 일을 반복하다 보면, 이 모종은 점점 안정적인 나만의 시스템으로 자라납니다.

## 어디서 시작하면 되나요?

- **처음 실행하기**: [`START_HERE.md`](START_HERE.md)
- **파트 3 실습**: [`lecture/exercises/part3_실습가이드.md`](lecture/exercises/part3_실습가이드.md)
- **파트 4 전략 실습**: [`lecture/exercises/part4_실습가이드.md`](lecture/exercises/part4_실습가이드.md)
- **API 키와 구독 연결**: [`docs/api-keys.md`](docs/api-keys.md)
- **KIS 모의·실전 설정**: [`docs/runtime-profiles.md`](docs/runtime-profiles.md)
