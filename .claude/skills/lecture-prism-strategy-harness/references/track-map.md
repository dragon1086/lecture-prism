...
- Broker config: `LECTURE_BROKER`, `LECTURE_TRADE_MODE`, 로컬 `.env`의 `KIS_PAPER_*` / `KIS_REAL_*`
...
## System Completion Lane is not a strategy track

A/B/C/D 중 하나의 데모 검증이 끝난 뒤에만 `system-completion.md`를 읽는다. 여기서는 전략을 더 수정하지 않고 같은 `run_id`의 `data_as_of`, 이벤트 `sequence`, Discord·선택 Telegram, 주문 상태, 대시보드 증거를 맞춘다.
