...
lecture-prism의 기본 성공 기준은 여전히 **API 키 없이 데모 모드에서 전략이 검증되는 것**입니다. 다만 수강생이 한국투자증권(KIS)이 아니라 키움증권, 토스증권, 다른 증권사 API를 준비해 오는 경우를 위해 브로커를 갈아끼울 수 있는 얇은 어댑터 구조를 제공합니다.

## 1. 전체 구조

| 위치 | 역할 |
|---|---|
| `trading.py` | 전략이 만든 공통 주문 객체를 선택 브로커로 넘깁니다. |
...
| 환경변수 | 의미 |
|---|---|
| `LECTURE_ENABLE_LIVE_BROKER=1` | demo/mock 브로커 API 호출을 허용합니다. |
| `LECTURE_ALLOW_REAL_BROKER=1` | real/prod/live 실전 모드를 추가로 허용합니다. |
...
2026년 6월 28일 기준으로 확인 가능한 토스 공식 개발자 문서는 토스페이먼츠 결제 API 중심이며, 토스증권의 공개 개인용 주문 API 문서는 확인하지 못했습니다. 그래서 `brokers/toss.py`는 **주문을 보내지 않는 안전 템플릿**입니다.

토스증권 파트너/비공개 API 문서를 수강생이 가져온 경우에는 아래 프롬프트로 확장합니다.
